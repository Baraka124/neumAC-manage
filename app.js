// app.js - Main Vue Application with all Templates
...
const app = createApp({
  setup() {
    ...
    // Login form (kept locally for reactivity)
    ...
    const researchLineFilters = reactive({
      search: '', active: ''
    })
    ...
    const medicalStaffModal = reactive({
      show: false, mode: 'add', activeTab: 'basic', _addingHospital: false,
      _newHospitalName: '', _newHospitalNetwork: 'external', _certs: [],
      _addingCert: false, _newCert: { name: '', issued_month: '', renewal_months: 24 },
      _addingStaffType: false, _newStaffTypeName: '', _newStaffTypeIsResident: false,
      ...
      form: {
        ...
        training_year: '', clinical_certificate: '', certificate_status: '', mobile_phone: '',
        medical_license: '', can_supervise_residents: false, special_notes: '', can_be_pi: false,
        can_be_coi: false, other_certificate: '', resident_category: null, home_department: null,
        external_institution: null, home_department_id: null, external_contact_name: null,
        external_contact_email: null, external_contact_phone: null, academic_degree_id: null,
        has_medical_license: false, residency_start_date: null, residency_year_override: null,
        is_chief_of_department: false, is_research_coordinator: false, is_resident_manager: false,
        is_oncall_manager: false, clinical_study_certificates: [], hospital_id: null,
        has_phd: false, phd_field: '', office_phone: '', years_experience: null,
        _networkHint: null, _coordLineId: null, _investigadorLines: []
      }
    })
    
    const rotationModal = reactive({
      show: false, mode: 'add', checkingAvailability: false, availability: null,
      form: {
    ...
    const onCallModal = reactive({
      ...
      form: {
        ...
        start_time: '15:00', end_time: '08:00', primary_physician_id: '',
    ...
    const absenceResolutionModal = reactive({
      show: false, absence: null, action: null, saving: false,
      returnDate: Utils.localDateStr(new Date()), returnNotes: '',
    ...
    const researchLineModal = reactive({
      show: false, mode: 'add',
      form: {
        line_number: null, research_line_name: '', name: '', description: '',
        capabilities: '', sort_order: 0, active: true, keywords: [], keywordsInput: ''
      }
    })
    ...
    const clinicalTrialModal = reactive({
      show: false, mode: 'add',
      form: {
        protocol_id: '', title: '', research_line_id: '', phase: 'Phase III',
        status: 'Reclutando', description: '', inclusion_criteria: '', exclusion_criteria: '',
    ...
    const innovationProjectModal = reactive({
      show: false, mode: 'add',
      form: {
        title: '', category: 'Dispositivo', current_stage: 'Idea', description: '',
        clinical_rationale: '', research_line_id: '', lead_investigator_id: '',
        co_investigators: [], partner_needs: [], partner_found: false, partner_name: '',
        funding_status: 'not_applicable', keywords: [], keywordsInput: '',
    ...
    const newsModal = reactive({
      ...
      form: {
        ...
        author_id: '', research_line_id: '', is_public: false, status: 'draft',
        expires_at: '', journal_name: '', authors_text: '', doi: ''
      }
    })
    ...
    const reassignmentModal = reactive({
      ...
      affectedShifts: [], affectedRotations: [], affectedAbsences: [], replacements: {}
    })
    const deptReassignModal = reactive({
      show: false, dept: null, impact: { activeStaff: [], activeUnits: [], activeRotations: [] },
      staffTargetDeptId: '', unitsTargetDeptId: ''
    })
    
    const staffTypeModal = reactive({
      show: false, mode: 'add', saving: false,
    ...
    const communicationsModal = reactive({
      ...
      form: { id: null, title: '', content: '', priority: 'normal', target_audience: 'all_staff' }
    })
    ...
    const viewStaffDetails = async (staff) => {
      ...
      if (hasPermission('analytics', 'read')) {
        ...
        try {
          const fullProfile = await API.getStaffResearchProfile(staff.id)
          if (fullProfile) staffProfileModal.researchProfile = fullProfile
        } finally { staffProfileModal.loadingResearch = false }
      }
      if (staff.staff_type === 'attending_physician') {
        staffProfileModal.loadingSupervision = true
        try {
          staffProfileModal.supervisionData = await API.getSupervisedResidents(staff.id)
        } finally { staffProfileModal.loadingSupervision = false }
      }
    }
    ...
    const showAddMedicalStaffModal = (opts = {}) => {
      ...
      medicalStaffModal.form = {
        ...
        office_phone: '', years_experience: null, _networkHint: null, _coordLineId: null,
        _investigadorLines: []
      }
    ...
    const deleteMedicalStaff = (staff) => {
      uiStore.showConfirmation({
        ...
        onConfirm: async () => {
          ...
          uiStore.showToast('Done', `${staff.full_name} has been deactivated`, 'success')
        }
      })
    }
    ...
    const deleteRotation = (rotation) => {
      uiStore.showConfirmation({
        title: 'Terminate Rotation',
        message: 'This will mark the rotation as terminated early.',
        icon: 'fa-stop-circle',
        confirmButtonText: 'Terminate',
        confirmButtonClass: 'btn-danger',
        details: `Resident: ${getResidentName(rotation.resident_id)}`,
        onConfirm: async () => {
          await rotationStore.deleteRotation(rotation.id)
          uiStore.showToast('Success', 'Rotation terminated', 'success')
        }
      })
    }
    ...
    const viewRotationDetails = (rotation) => {
      rotationViewModal.rotation = {
        ...
        supervisorName: getStaffName(rotation.supervising_attending_id) || '—',
    ...
    const editOnCallSchedule = (schedule) => {
      ...
      onCallModal.form = {
        ...
        shift_type: ['primary', 'primary_call'].includes(schedule.shift_type) ? 'primary_call' : 'backup_call'
      }
      onCallModal.show = true
    }
    ...
    const deleteOnCallSchedule = (schedule) => {
      uiStore.showConfirmation({
        ...
        onConfirm: async () => {
          await onCallStore.deleteOnCall(schedule.id)
          uiStore.showToast('Success', 'Schedule deleted', 'success')
        }
      })
    }
    ...
    const deleteAbsence = (absence) => {
      uiStore.showConfirmation({
        ...
        message: 'This will mark the absence as cancelled.',
        icon: 'fa-ban',
        ...
        onConfirm: async () => {
          ...
          uiStore.showToast('Success', 'Absence record cancelled', 'success')
        }
      })
    }
    ...
    const openUnitClinicians = (unit) => {
      ...
      unitCliniciansModal.allStaff = staffStore.medicalStaff.filter(s =>
        (s.staff_type === 'attending_physician' || s.staff_type === 'fellow') && s.employment_status === 'active'
      )
      ...
      try {
        if (communicationsModal.activeTab === 'announcement') {
          ...
          if (communicationsModal.mode === 'edit') {
            ...
            uiStore.showToast('Success', 'Announcement updated', 'success')
          } else {
            await API.createAnnouncement({ title: f.title, content: f.content, priority_level: f.priority, target_audience: f.target_audience })
            uiStore.showToast('Success', 'Announcement posted', 'success')
          }
          await API.getAnnouncements().then(data => { announcements.value = data })
        }
    ...
    const deleteResearchLine = (line) => {
      uiStore.showConfirmation({
        ...
        confirmButtonClass: 'btn-danger',
        onConfirm: async () => {
          await researchStore.deleteResearchLine(line.id)
          uiStore.showToast('Success', 'Research line deleted', 'success')
        }
      })
    }
    ...
    const deleteClinicalTrial = (trial) => {
      uiStore.showConfirmation({
        ...
        confirmButtonClass: 'btn-danger',
        onConfirm: async () => {
          await researchStore.deleteClinicalTrial(trial.id)
          uiStore.showToast('Success', 'Study deleted', 'success')
        }
      })
    }
    ...
    const deleteInnovationProject = (project) => {
      uiStore.showConfirmation({
        ...
        confirmButtonClass: 'btn-danger',
        onConfirm: async () => {
          await researchStore.deleteInnovationProject(project.id)
          uiStore.showToast('Success', 'Project deleted', 'success')
        }
      })
    }
    ...
    const openLineDetail = (line) => {
      researchStore.setActiveMissionLine(line)
    }
    
    const drillToTrials = (lineId) => {
      trialFilters.line = lineId
      researchHubTab.value = 'trials'
    }
    
    const drillToProjects = (lineId) => {
      projectFilters.research_line_id = lineId
      researchHubTab.value = 'projects'
    }
    ...
    const deleteNews = (post) => {
      uiStore.showConfirmation({
        ...
        confirmButtonClass: 'btn-danger',
        onConfirm: async () => {
          await newsStore.deleteNews(post.id)
          uiStore.showToast('Deleted', 'Post deleted', 'success')
        }
      })
    }
    ...
    const switchView = async (view, filters = {}) => {
      ...
      if (filters.residentCategory && staffFilters) {
        staffFilters.staffType = 'medical_resident'
        staffFilters.residentCategory = filters.residentCategory
      }
    ...
    const goToPage = (view, page) => {
      if (pagination[view]) pagination[view].page = page
    }
    ...
    const handleLogin = async () => {
      loginFieldErrors.value.email = !loginForm.value.email ? 'Email required' : ''
      loginFieldErrors.value.password = !loginForm.value.password ? 'Password required' : ''
      if (loginFieldErrors.value.email || loginFieldErrors.value.password) {
    ...
    const handleLogout = () => {
      uiStore.showConfirmation({
        ...
        onConfirm: async () => {
          ...
          uiStore.showToast('Info', 'Logged out successfully', 'info')
        }
      })
    }
    ...
    const updateDashboardStats = () => {
      ...
      systemStats.onLeaveStaff = absenceStore.absences.filter(a => {
        ...
        return start <= today && today <= end && a.current_status !== 'cancelled'
      }).length
      ...
      if (endingThisWeek.length > 0) {
        items.push({ type: 'warn', text: `${endingThisWeek.length} rotation${endingThisWeek.length > 1 ? 's' : ''} ending this week`, action: 'resident_rotations', icon: 'fa-clock' })
      }
      ...
      if (startingThisWeek.length > 0) {
        items.push({ type: 'ok', text: `${startingThisWeek.length} rotation${startingThisWeek.length > 1 ? 's' : ''} starting this week`, action: 'resident_rotations', icon: 'fa-play-circle' })
      }
      const ocGaps = []
      for (let i = 0; i < 7; i++) {
        const d = new Date(Date.now() + i * 86400000)
        const ds = Utils.normalizeDate(d)
        const hasPrimary = onCallStore.onCallSchedule.some(s => Utils.normalizeDate(s.duty_date) === ds && s.shift_type === 'primary_call')
        if (!hasPrimary) ocGaps.push(d.toLocaleDateString('es-ES', { weekday: 'short', day: 'numeric', month: 'short' }))
      }
      if (ocGaps.length > 0) {
        items.push({ type: 'danger', text: `${ocGaps.length} day${ocGaps.length > 1 ? 's' : ''} without primary on-call`, action: 'oncall_schedule', icon: 'fa-phone-slash' })
      }
    ...
    const filteredMedicalStaff = computed(() => {
      ...
      const field = sortState.medical_staff.field
      const dir = sortState.medical_staff.dir
      return [...f].sort((a, b) => {
    ...
    const filteredRotations = computed(() => {
      ...
      const field = sortState.rotations.field
      const dir = sortState.rotations.dir
      return [...f].sort((a, b) => {
        let va = a[field] ?? '', vb = b[field] ?? ''
    ...
    const filteredOnCallSchedules = computed(() => {
      ...
      const field = sortState.oncall.field
      const dir = sortState.oncall.dir
      return [...f].sort((a, b) => {
        let va = a[field] ?? '', vb = b[field] ?? ''
        if (field === 'duty_date') {
          va = Utils.normalizeDate(va)
    ...
    const filteredAbsences = computed(() => {
      ...
      const field = sortState.absences.field
      const dir = sortState.absences.dir
      return [...f].sort((a, b) => {
        let va = a[field] ?? '', vb = b[field] ?? ''
        if (field === 'start_date' || field === 'end_date') {
          va = Utils.normalizeDate(va)
    ...
    const filteredResearchLines = computed(() => {
      let f = researchStore.researchLines
    ...
    return {
      // Stores
      uiStore, userStore, staffStore, rotationStore, trainingStore, onCallStore,
      absenceStore, researchStore, newsStore,
      ...
      staffFilters, rotationFilters, onCallFilters, absenceFilters,
      trainingUnitFilters, researchLineFilters, trialFilters, projectFilters, newsFilters,
      pagination, sortState, sortBy, sortIcon,
      
      // Modals
      staffProfileModal, medicalStaffModal, rotationModal, rotationViewModal,
      onCallModal, absenceModal, absenceResolutionModal, trainingUnitModal,
      unitResidentsModal, unitCliniciansModal, unitDetailDrawer, occupancyPanel,
      tlPopover, researchLineModal, clinicalTrialModal, innovationProjectModal,
      assignCoordinatorModal, trialDetailModal, newsModal, newsDrawer,
      deptPanel, reassignmentModal, deptReassignModal, staffTypeModal,
      exportModal, userProfileModal, confirmationModal, announcementReadModal,
      ...
      dailyBriefing, todaysOnCall, upcomingOnCallDays, residentGapWarnings,
      announcements,
      ...
      formatRelativeTime, formatNewsDate, getInitials, getStaffName, getTrainingUnitName,
      getResidentName, getSupervisorName, getPhysicianName, getDepartmentName,
      getResearchLineName, getPhaseColor, getStageColor, getStageConfig,
      getPartnerTypeColor, getLineAccent, formatStaffType, formatStaffTypeShort,
      getStaffTypeClass, isResidentType, formatEmploymentStatus, formatAbsenceReason,
      formatRotationStatus, formatStudyStatus, formatAudience, calculateAbsenceDuration,
      getDaysRemaining, getDaysUntilStart, getRotationProgress, getCurrentRotationForStaff,
      getRotationHistory, getUpcomingRotations, getRotationDaysLeft, getCurrentRotationSupervisor,
      isOnCallToday, getUpcomingOnCall, getUpcomingLeave, getUnitActiveRotationCount,
      getUnitRotations, getUnitScheduledCount, getUnitOverlapWarning, getUnitMonthOccupancy,
      getNextFreeMonth, getTimelineMonths, getUnitSlots, getDaysUntilFree,
      ...
      filteredAbsences, filteredTrainingUnits, filteredResearchLines,
      filteredTrials, filteredProjects, filteredNews,
